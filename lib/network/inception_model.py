...
class InceptionBackBone(object):

    def inception_base(self, inputs):
        featuremap_scale = 1
        ...
        net = self._inception_module(net)
        net = slim.max_pool2d(net, [2, 2], stride=2, padding='VALID', scope='pool2')
        featuremap_scale *= 2

        return net, featuremap_scale

    def _inception_module(self, net, module_index=None):
        """
        inception_module
        :param net:
        :param module_index:
        ...
        with tf.variable_scope(scope):
            ...
            net = tf.concat([branch_0, branch_1, branch_2, branch_3], axis=3, name=scope + '/concat')
